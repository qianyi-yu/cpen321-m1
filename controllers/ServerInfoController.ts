...
export class ServerInfoController {
    ...
    async getServerTime(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            // Convert the current time to Eastern Time (America/New_York)
            const easternTime = now.toLocaleString("en-US", { timeZone: "America/New_York" });
    
            // Parse the converted time to extract the desired format
            const easternDate = new Date(easternTime);
    
            // Format the time in hh:mm:ss
            const formattedTime = easternDate.toISOString().split("T")[1].split(".")[0]; // Extract hh:mm:ss
    
            // Calculate GMT offset for Eastern Time
            const timeZoneOffset = easternDate.getTimezoneOffset(); // Offset in minutes
            ...
            // Combine formatted time and GMT offset
